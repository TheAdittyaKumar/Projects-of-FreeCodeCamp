...
const getChange = (changeDue, cid) => {
  ...
  // Special case: If remaining change is exactly 0 and the total cash used equals all the cash in drawer
  if (parseFloat(remainingChange) === 0 && totalCid === changeDue) {
...
const formatChange = (changeArray) => 
  changeArray
    .filter(([unit, amount]) => amount > 0)  // Filters out zero-amount units
    .map(([unit, amount]) => `${unit}: $${amount.toFixed(2)}`)
    .join(" ");


// Test example based on your requirements:
console.log(getChange(19.5, [['PENNY', 0.5], ['NICKEL', 0], ['DIME', 0], ['QUARTER', 0], ['ONE', 0], ['FIVE', 0], ['TEN', 0], ['TWENTY', 0], ['ONE HUNDRED', 0]]));
console.log(getChange(19.5, cid));
